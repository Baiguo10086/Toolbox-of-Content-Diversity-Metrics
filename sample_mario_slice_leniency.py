...
class GamePattern:
    def __init__(self,height:float,st:int) -> None:
        self.height=height
        self.st=st
        self.flag=False
    ...
    def set_ed(self,ed:int):
        self.ed=ed
        self.pos=(self.st,ed)

def is_not_empty(item:int) -> bool:
    #  'c-i': {'X': 0, 'S': 1, '-': 2, '?': 3, 'Q': 4, 'E': 5, '<': 6,
    #         '>': 7, '[': 8, ']': 9, 'o': 10}
    return item==0 or item==1 or item == 3 or item ==4 or item ==6 or item ==7 or item ==8 or item ==9 

def splict_level(game_level_2d:GameLevel2D) ->List[GamePattern]:
    lst=-2
    st=0
    game_patterns:List[GamePattern]=[]
    for column in range(game_level_2d.columns):
        height=-1
        for row in range(game_level_2d.rows):
            if is_not_empty(game_level_2d.map[row][column]):
                height=row
                break
        if height!=lst:
            if game_patterns:
                game_patterns[len(game_patterns)-1].set_ed(column-1)
            game_patterns.append(GamePattern(height,column))
            lst=height
    
    game_patterns[len(game_patterns)-1].set_ed(game_level_2d.columns-1)
    for i in range(len(game_patterns)):
        for column in range(game_patterns[i].st,game_patterns[i].ed+1):
            for row in range(game_level_2d.rows):
                if game_level_2d.map[row][column]==5:
                    game_patterns[i].set_danger()
                    break
            if game_patterns[i].flag:
                break
...
if __name__ == '__main__':
    ...
    for i in range(99):
        ...
        if os.path.exists(file_name):
            ...
            game_patterns=splict_level(game_level_2d=level)
            img = lvl.to_img()
            for game_pattern in game_patterns:
                st=((game_pattern.st)*tex_size,(game_pattern.height)*tex_size)
                ed=((game_pattern.ed+1)*tex_size,(game_pattern.height)*tex_size)
                
                if game_pattern.height==-1:
                    st=(st[0],(level.rows)*tex_size)
                    ed=(ed[0],(level.rows)*tex_size)
                
                if game_pattern.flag:
                    pg.draw.line(img,RED,st,ed,3)
                else:
                    pg.draw.line(img,GREEN,st,ed,3)
            ...
            save_img(img,PROJ_DIR+f'/levels/original/slice/mario-{i}.png')
            # result=GameLevel2D.calculate_linearity(level,weight_dict,calculate_linear_regression_least_squares)
            result=0
            for game_pattern in game_patterns:
                if game_pattern.ed-game_pattern.st+1>=5:
                    result-=2
            for t in range(len(game_patterns)-1):
                if game_patterns[t].height!=-1:
                    if game_patterns[t+1].height==-1:
                        result+=1
                if game_patterns[t+1].flag:
                    result+=1
                if game_patterns[t].height!=-1 and game_patterns[t+1].height!=-1:
                    result-=1
